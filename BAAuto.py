...
class Config:
    def __init__(self, config_file):
        ...
        self.widgets = self.set_values_to_none(self.config_data)
        ...
        for line in iter(self.p.stdout.readline, b''):
            ...
            for level, color in self.log_textbox.log_level_colors.items():
                if level in line:
                    ...
                    break
            if not colored:
                # Display output in text box without color
                pass
...
class Sidebar(customtkinter.CTkFrame):
    ...
    def create_module_frames(self):
        ...
        for index, sublist in enumerate(self.module_list):
            module = sublist[0]
            self.gui_dictionary[module] = {}
            self.create_module_checkbox(module, index)
            self.create_module_button(module, index)
            frame = sublist[1](self.master, self.config, fg_color="#262250") 
            self.gui_dictionary[module]['frame'] = frame
        self.gui_dictionary["login"]["button"].configure(image=self.gear_on)
        self.gui_dictionary["login"]["checkbox"].configure(text_color="#53B9E9")   
        self.current_frame = self.gui_dictionary["login"]["frame"]  # Update the current frame
        self.current_frame.grid(row=0, column=1, padx=20, pady=20, sticky="nsew")

    def create_module_checkbox(self, module, i):
        self.gui_dictionary[module]['checkbox'] = customtkinter.CTkCheckBox(
        ...
        self.gui_dictionary[module]['checkbox'].grid(row=i, column=0, columnspan=2,padx=20, pady=(10, 5), sticky="nw")
        self.config.widgets[module]['enabled'] = self.gui_dictionary[module]['checkbox']

    def create_module_button(self, module, i):
        self.gui_dictionary[module]['button'] = customtkinter.CTkButton(
        ...
        self.gui_dictionary[module]['button'].grid(row=i, column=1, padx=(40,0), pady=(2,0), sticky="nw")        
    ...
    def create_start_button(self):
        self.start_button = customtkinter.CTkButton(self, text="Start", width=180, height=40, command=self.config.start_stop, font=customtkinter.CTkFont(family="Inter", size=16))
        self.start_button.grid(row=2, column=0, padx=(35,0), pady=(25,0),  sticky="nsew")
        self.config.start_button = self.start_button

    def select_all(self):
        for module in self.gui_dictionary:
            if module != "momotalk":
                self.gui_dictionary[module]["checkbox"].select()
                self.config.config_data[module]["enabled"] = True
        self.config.save_file()

    def clear_all(self):
        for module in self.gui_dictionary:
            self.gui_dictionary[module]["checkbox"].deselect()
            self.config.config_data[module]["enabled"] = False
        self.config.save_file()

    def display_settings(self, module):
        for key in self.gui_dictionary:
            if key == module:
                self.gui_dictionary[key]["button"].configure(image=self.gear_on)
                self.gui_dictionary[key]["checkbox"].configure(text_color="#53B9E9")                
                self.current_frame.grid_remove()  # Hide the current frame
                self.current_frame = self.gui_dictionary[key]["frame"]  # Update the current frame
                self.current_frame.grid(row=0, column=1, padx=20, pady=20, sticky="nsew")
            else:
                self.gui_dictionary[key]["button"].configure(image=self.gear_off)
                self.gui_dictionary[key]["checkbox"].configure(text_color="#FFFFFF")  
...
class LoginFrame(customtkinter.CTkFrame):
    ...
    def create_network_widgets(self):
        ...
        self.config.widgets["login"]["network"] = self.network_entry
    ...
    def create_screenshot_widgets(self):
        ...
        self.config.widgets["login"]["screenshot_mode"] = self.screenshot_dropdown
    ...
    def create_restart_attempts_widgets(self):
        ...
        self.config.widgets["login"]["restart_attempts"] = self.restart_attempts_spinbox
...
class CafeFrame(customtkinter.CTkFrame):
    def __init__(self, master, config, **kwargs):
        ...
        self.config.widgets["cafe"]["invite_student"] = self.invite_checkbox
        self.config.widgets["cafe"]["student_name"] = self.student_entry
        self.config.widgets["cafe"]["tap_students"] = self.tap_checkbox
        self.config.widgets["cafe"]["claim_earnings"] = self.claim_checkbox
...
class FarmingFrame(customtkinter.CTkScrollableFrame):
    def __init__(self, master, config, **kwargs):
        ...
        self.config.widgets["farming"]["bounty"]["enabled"] = self.bounty_checkbox
        ...
        for index, element in enumerate(["overpass", "desert_railroad", "classroom"]):
            ...
            self.config.widgets["farming"]["bounty"][element]["stage"] = option_menu
            ...
            self.config.widgets["farming"]["bounty"][element]["run_times"] = spinbox
        ...
        self.config.widgets["farming"]["scrimmage"]["enabled"] = self.scrimmage_checkbox
        for index, element in enumerate(["trinity", "gehenna", "millennium"]):
            ...
            self.config.widgets["farming"]["scrimmage"][element]["stage"] = option_menu
            ...
            self.config.widgets["farming"]["scrimmage"][element]["run_times"] = spinbox
        ...
        self.config.widgets["farming"]["tactical_challenge"]["enabled"] = self.tactical_checkbox
        ...
        self.config.widgets["farming"]["tactical_challenge"]["rank"] = self.rank_optionmenu
        ...
        self.config.widgets["farming"]["mission"]["enabled"] = self.mission_commissions_checkbox
        ...
        self.config.widgets["farming"]["mission"]["reset_daily"] = self.reset_daily
        self.config.widgets["farming"]["mission"]["reset_time"] = self.reset_time
        ...
        self.config.widgets["farming"]["mission"]["recharge_ap"] = self.recharge_checkbox
        ...
        self.config.widgets["farming"]["mission"]["event"] = self.event_checkbox
        ...
        for i in [self.tab_queue, self.tab_template]:
            ...
            self.clear_button = customtkinter.CTkButton(self.template_buttons_frame, text="Clear All", command=lambda queue=queue: self.clear_frames(queue=queue))
...
class ClaimRewardsFrame(customtkinter.CTkFrame):
    def __init__(self, master, config, **kwargs):
        ...
        self.config.widgets["claim_rewards"]["tasks"] = self.claim_tasks_checkbox
        self.config.widgets["claim_rewards"]["mailbox"] = self.claim_mail_checkbox 
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        sidebar = Sidebar(self, config, fg_color="#25224F")
        sidebar.grid(row=0, column=0, sticky="nsw")
        logger = LoggerTextBox(self, config, fg_color="#262250")
